<div class="mb-20 flex">
	<img src="/profile.jpg" alt="logo" class="rounded-full w-40 mr-10" />
	<p class="justify-end font-bold text-3xl mt-10">Hello I'm Varun Pereira!</p>
</div>
<p id="skills" class="text-center font-bold text-2xl">Skills</p>
<p id="projects" class="text-center font-bold text-2xl">Projects</p>
<div class="flex flex-wrap text-white">
	<!-- 1 -->
	<a
		href="https://denify.vercel.app/"
        target="_blank"
		class="mr-4 max-w-[16rem] max-h-[32rem] text-sm rounded-lg bg-white hover:bg-gray-300 "
	>
		<img class="h-44 w-96 rounded-t-lg object-cover" src="/denify.jpg" alt="profile" />
		<div class="mb-5 px-5 pt-5">
			<p class="h-16  font-bold tracking-tight text-gray-900 ">Ecommerce Store</p>
			<p class="h-20 font-normal text-gray-700">Similar to Amazon.</p>
			<button
				class="cursor-pointer inline-flex w-full items-center justify-center rounded-lg bg-black py-2 m font-medium text-white "
			>
				Get Code
			</button>
		</div>
	</a>
</div>
